...
This contract supersedes the v2 execution philosophy for Harness Engineer runs. It preserves all v2 continuity, six-lane, LSP, checkpoint, recovery, artifact-only and quality requirements, but changes the closure model after repeated external-review escapes.
...
Exactly six logical lanes exist across the entire package lifetime, including recovery generations. A process/session restart does not create a new batch.
...
- formal semantic invariant in plain deterministic terms;
...
A family cannot be declared recertified while a material decision-changing dimension is merely untested, assumed safe, or represented by one example.
...
Any `MATERIAL_GAP` => candidate is BLOCKED.

A new witness from a family that was previously declared exhausted is not handled by growing a denylist/allowlist, adding one regex branch, adding one transition special case, or adding one test-only guard unless the FAMILY_MODEL proves that operation closes the complete equivalence class.
...
Before fan-out, establish one compact `SHARED_EVIDENCE_MAP` bound to exact START/TREE. Reuse it across recovery generations. At minimum it records:
...
- open hypotheses and the lane assigned to challenge each;
...
Lanes 1–5 investigate and construct the family models BEFORE implementation. Lane 6 is fundamentally different: it is reserved for POST-IMPLEMENTATION independent self-falsification and MUST NOT be completed before the final candidate exists.
...
Map prior closures and directly reachable neighboring families. Prove that the candidate design will preserve them. Do not restart unrelated history.
...
- LSP-after has been run on modified symbols and affected callers;
- the recovery patch reflects the current candidate.

Then instantiate a fresh adversarial subagent/context that did NOT propose or implement the patch. Give it the invariant/FAMILY_MODEL, exact final candidate, historical counterexamples and required regression laws, but do not give it the implementation rationale as proof.
...
A prior failed L6 attempt is evidence, not a completed lane. Only the first clean challenge of the final unchanged candidate may set L6=`COMPLETED`.
...
- all decision-changing dimensions disposed without `MATERIAL_GAP`;
- seed witnesses reproduced before and rejected after;
- generated neighboring classes/property tests pass;
- benign controls pass;
- historical closures remain intact;
- LSP-before and LSP-after evidence complete;
- L6 fresh post-implementation adversarial challenge PASS on the exact final patch;
- closure argument explains WHY the whole family is closed, not only WHAT tests pass.

The final closure report must contain a `RECURRENT FAMILY RECERTIFICATION MATRIX` with one row per dimension/class and its disposition/evidence.
...
4. Determine which families are recurrent and create FAMILY_MODEL(s).
5. Execute/inherit L1–L5 only. Do not start L6 yet.
6. Synthesize the family-complete correction design from L1–L5.
7. Implement the smallest COMPLETE family correction in allowed paths.
...
## SUBAGENT SWARM
Exactly six logical lanes. L1–L5 investigation evidence; L6 post-implementation independent challenge, including challenger attempts and final clean attempt.
...
Fresh L6 challenger identity/context separation, new probes, findings/dispositions, whether patch mutated after any failed attempt, and proof final L6 reviewed exact final patch.
