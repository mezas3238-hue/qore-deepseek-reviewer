QORE UMI-14 / UNR-020 — CODER REVIEW RETRY
...
Planner/tool law: use supplied evidence first. If extra evidence is genuinely required and `git_show` is used, its `ref` argument MUST be exactly one of the literal aliases `BASE`, `HEAD`, or `SYNTHETIC`; never pass a SHA, path, filename, or other ref. Keep planning bounded; do not request evidence already present in the package.
...
If evidence insufficient after valid bounded tooling, block.
